...
import 'posthog_http.dart';

// Differs from PostHog upstream: the `OnFeatureFlagsCallback` typedef moved to
// `posthog_config.dart` — its only user, `PostHogConfig`, lives there, and this
// removes the config's dependency on the platform interface. The public API is
// unchanged: both are exported from `posthog.dart`.
...
abstract class PosthogFlutterPlatformInterface extends PlatformInterface {
  ...
  // Differs from PostHog upstream: the default instance is `PosthogHttp`, a
  // pure-Dart HTTP transport, rather than `PosthogFlutterIO` (MethodChannel).
  static PosthogFlutterPlatformInterface _instance = PosthogHttp();

  /// The default instance of [PosthogFlutterPlatformInterface] to use.
  ///
  /// Defaults to [PosthogHttp].
  ...
  ///
  /// Differs from PostHog upstream: always a no-op here. Detecting and
  /// capturing native screens requires the native SDK. Kept for API
  /// compatibility.
