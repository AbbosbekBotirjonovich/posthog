...
class PostHogDisplaySurvey {
  ...
  factory PostHogDisplaySurvey.fromDict(Map<String, dynamic> dict) {
    final questions = (dict['questions'] as List).map((q) {
      // Differs from PostHog upstream: the official plugin mistakenly read
      // `id` from `q['type']`, even though the native side sent `id`
      // separately. That corrupted the response keys
      // (`$survey_response_<id>`).
      final id = q['id'] as String? ?? '';
      final type = q['type'] as String;
      final question = q['question'] as String;
      // Differs from PostHog upstream: this was a non-null cast. A default is
      // used so an unexpected payload cannot crash the whole survey.
      final optional = q['isOptional'] as bool? ?? false;
      ...
      // Differs from PostHog upstream: the official plugin never read
      // branching (the native SDK picked the next question). In pure Dart that
      // decision is made in `SurveyBranching`, so the raw condition is kept.
      final branching = q['branching'] is Map
          ? Map<String, dynamic>.from(q['branching'] as Map)
          : null;
      ...
      switch (type) {
        ...
        case 'rating':
          // Differs from PostHog upstream: these fields were non-null casts.
          // PostHog's standard 1-5 scale is used as the default so an
          // incomplete payload cannot crash the whole survey.
          return PostHogDisplayRatingQuestion(
            id: id,
            question: question,
            ratingType: PostHogDisplaySurveyRatingType.fromInt(
              q['ratingType'] as int? ?? 0,
            ),
            scaleLowerBound: q['scaleLowerBound'] as int? ?? 1,
            scaleUpperBound: q['scaleUpperBound'] as int? ?? 5,
        ...
        case 'single_choice':
          return PostHogDisplayChoiceQuestion(
            ...
            question: question,
            choices:
                (q['choices'] as List?)?.cast<String>() ?? const <String>[],
            isMultipleChoice: type == 'multiple_choice',
            hasOpenChoice: q['hasOpenChoice'] as bool? ?? false,
            shuffleOptions: q['shuffleOptions'] as bool? ?? false,
            description: questionDescription,
            descriptionContentType: questionDescriptionContentType,
            optional: optional,
            buttonText: buttonText,
